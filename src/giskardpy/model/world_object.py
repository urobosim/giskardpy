...
class WorldObject(URDFObject):
    def init2(self, base_pose=None, controlled_joints=None, path_to_data_folder=u'',
        ...
        if not ignored_pairs:
            ...
            # turn lists to tuple
            self._ignored_pairs = {tuple(x) if isinstance(x, list) else x for x in ignored_pairs}
        if not added_pairs:
            self._added_pairs = set()
        else:
            self._added_pairs = {tuple(x) if isinstance(x, list) else x for x in added_pairs}
    ...
    def get_joint_position_symbol(self, joint_name):
        return self.get_god_map().identifier_to_symbol(identifier.km_world + [u'data_tree', u'data_tree',
    ...
    def calc_collision_matrix(self, link_combinations=None, d=0.05, d2=0.0, num_rnd_tries=2000):
        ...
        for link_a, link_b in link_combinations:
            if self.are_linked(link_a,link_b) or \
                    link_a == link_b or \
                    link_a in self._ignored_pairs or \
                    link_b in self._ignored_pairs or \
                    (link_a, link_b) in self._ignored_pairs or \
